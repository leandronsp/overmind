...
| **M3** | **Declarative Config** (done) | Blueprint TOML. `overmind apply` (async, returns immediately), `overmind agents`. K8s-like semantics. `depends_on` DAG with cycle detection. Runner GenServer registers in ETS as `:blueprint` type — all existing commands (wait, logs, ps, stop, kill) work on blueprints. `kill --all`, `logs` (all agents). | Partially --- others have config, not the k8s abstraction + deterministic orchestration |
